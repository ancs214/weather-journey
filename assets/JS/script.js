...
let historySearch = function (event) {
  ...
  cityHeaderEl.innerHTML =
    hxSearchEntry + '&nbsp' + moment().format('(MM/DD/YY)')
  ...
  fetch(
    'https://api.openweathermap.org/geo/1.0/direct?q=' +
      city +
      '&limit=10&appid=314928b8d3e16dd41afeacded00b5a17'
...
let displayStates = function (data) {
  ...
  for (let i = 0; i < data.length; i++) {
    ...
    stateBtn.onclick = function () {
      ...
      cityHeaderEl.innerHTML =
        locationEl + '&nbsp' + moment().format('(MM/DD/YY)')
    }
  }
}
...
let getCurrentWeather = function (lat, lon) {
  fetch(
    'https://api.openweathermap.org/data/2.5/onecall?lat=' +
      ...
      '&lon=' +
      lon +
      '&units=imperial&exclude={part}&appid=314928b8d3e16dd41afeacded00b5a17'
  ).then(function (response) {
    if (response.ok) {
      response.json().then(function (data) {
